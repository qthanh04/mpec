...
$(function () {
    ...
    viewCongNo();
    clickSearchCongNoKhachHang();
    checkThoiGianTHDN();
});

//=================== Function ===================//
function clickSearchCongNoKhachHang() {
    btnTimKiem.click(function () {
        ...
        if (check) {
            ...
            CongNoKhachHang(valTuNgay,valDenNgay,text);
        }
...
//=================== Function ===================//
function CongNoKhachHang(valTuNgay,valDenNgay,text) {
    $("#pageBody").LoadingOverlay("show");
    searchCongNoKhachHang(valTuNgay,valDenNgay,text,1,10).then(rs => {
        if (rs.message === "found") {
            $('#pagination').pagination({
                ...
                callback: function (response, pagination) {
                    ...
                    searchCongNoKhachHang(valTuNgay,valDenNgay,text, pagination.pageNumber, 10).then(rs => {
                        arr = rs.data.currentElements;
...
function viewCongNo(ngayDau,ngayCuoi) {
    $("#pageBody").LoadingOverlay("show");
    findCongNoKhachHang(ngayDau,ngayCuoi,1,10).then(rs => {
        if (rs.message === "found") {
            $('#pagination').pagination({
                dataSource: function (done) {
                    let result = [];
                ...
                callback: function (response, pagination) {
                    ...
                    // console.log(pagination.pageNumber); // khi click sẽ đọc ra số trang click
                    findCongNoKhachHang(ngayDau,ngayCuoi, pagination.pageNumber, 10).then(rs => {
                        arr = rs.data.currentElements;
                        setViewCongNo(pagination.pageNumber);
                    }).catch(err => console.log(err))
                }
...
function setViewCongNo(pageNumber) {
    ...
    if (len > 0) {
        view += arr.map((item, index) => `<tr data-index="${index}" class="click-thuong-hieu">
                    ...
                    <td>${viewField(item.tongTienMua)}</td>
